...
#### Upgrade custom authentication methods

`Authenticator` doesn't provide the name of the form anymore, this is now moved to the Form itself, requiring `getAuthenticatorName()` on the abstract `LoginForm` class. Any naming of custom authenticators should live there.

The move to `LoginForm` providing the name gives developers more flexibility on how they want to name their form, without having to extend `Authenticator` to just override the name of the login form.

An authentication form requires an authenticator class being passed into the constructor. In a custom Authenticator class, this is set in `get_login_form()` method, passing in itself as a string as second argument.

Authentication no longer accepts an empty string or an unset Authentication Method. 
Registering or unregistering an Authenticator can be done in the `_config.php`

```
Authenticator::register_authenticator('MyVendor\\MyModule\\MyAuthenticator');

Authenticator::unregister_authenticator('SilverStripe\\Security\\MemberAuthenticator');
```

Or register your Authenticator via YML config:
```yaml
...
Instead of relying on `LoginForm::__construct()`, it is now possible to add or remove fields or actions with respectively `getFormFields()` or `getFormActions()` methods. Removing the overhead of the constructor and making the process of overriding the default `MemberLoginForm` easier.

Any `LoginForm` extension requires these methods to be implemented. Extending `MemberLoginForm` is enough if it is other parts of the Form that require adjustments. Direct descendants of `LoginForm` however, require these methods to build the form.
...
* Removed `get_name` from `Authenticator` in favor of `getAuthenticatorName` on `LoginForm`
 ...
 * `MemberLoginForm` has a new constructor argument for the authenticator class, athough this is usually
 ...
 * Direct descendants of `LoginForm` require a `getFormFields` and `getFormActions` method to implement the required form.
 * `on_register` and `un_register` have been removed. `Authenticator` will no longer check if an Authentication method is allowed to be registered or unregistered. History shows it is never used and therefore only added unneeded overhead.
